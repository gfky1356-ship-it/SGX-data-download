...
class GlobalScreenerV59_CN:
    ...
    def generate_html(self):
        ...
        if not datasource_rows:
            datasource_rows = '<div>（自动扫描模式 — 数据来源：Yahoo Finance / SEC EDGAR）</div>'
        css = f"""
...
def _fetch_fa_data(yf_ticker, fallback_name):
    ...
    return {
        "company_name":      yf_info.get("longName") or yf_info.get("shortName") or fallback_name,
        ...
        "_pe_source":        "Yahoo" if yf_pe is not None else "N/A",
        "financial_reports": [],
    }
