...
# Benchmark : SPY  (TradingView: SPY)
# Filters   : volume > 200,000  AND  price > US$10.00
#
# Scanners:
...
#   us_scan_results.csv
#   us_watchlist.txt              ← import into TradingView
#   us_rsi_comparison.csv         ← RSI14 ratio vs SPY
#   benchmark_data/SPY.csv
#   fa_reports/<TICKER>_FA_v6.0_<date>.html
#   fa_reports/US_FA_REPORTS_<date>.pdf
# ============================================================
...
# ── Output paths ─────────────────────────────────────────────
OUTPUT_RESULTS    = os.path.join(GDRIVE_BASE, "us_scan_results.csv")
OUTPUT_WATCHLIST  = os.path.join(GDRIVE_BASE, "us_watchlist.txt")
OUTPUT_RSI        = os.path.join(GDRIVE_BASE, "us_rsi_comparison.csv")
FA_REPORTS_DIR    = os.path.join(GDRIVE_BASE, "fa_reports")
COMBINED_FA_PDF   = os.path.join(FA_REPORTS_DIR, f"US_FA_REPORTS_{datetime.now().strftime('%Y%m%d')}.pdf")
...
def run_fa_scan(matched, ticker_info):
    ...
    for r in matched:
        ...
        user_msg = (
            ...
            "1. Apply US-specific benchmarks (EPS growth ≥10%, Revenue CAGR >10%, ROE >15%, etc.)\n"
            "2. Retrieve the latest financial data from SEC EDGAR / Yahoo Finance\n"
            "3. Score each metric against the US benchmarks\n"
...
print(f"  Filters: vol > {MIN_VOLUME:,}  |  price > US${MIN_PRICE}")
print(f"  Benchmark: {BENCHMARK}  (TradingView: SPY)")
print("=" * 60)

# Step 1: Fetch S&P 500 ticker list from Wikipedia
print("\n[1/5] Fetching S&P 500 stock list from Wikipedia...")
try:
    ...
    ticker_info = {
        row["Symbol"]: {"symbol": row["Symbol"], "name": row["Security"], "sector": row.get("GICS Sector", "")}
        for _, row in sp500_table.iterrows()
    }
    print(f"    Found {len(ticker_info)} S&P 500 stocks")
except Exception as e:
    print(f"    Wikipedia fetch failed ({e}), falling back to yfinance...")
    import json
    resp = requests.get(
    ...
    stocks = resp.json()
    ticker_info = {s["Symbol"]: {"symbol": s["Symbol"], "name": s["Name"], "sector": s.get("Sector", "")} for s in stocks}
    print(f"    Found {len(ticker_info)} S&P 500 stocks (fallback source)")
...
save_benchmark_to_csv(bench_close, "SPY", BENCHMARK_DATA_DIR)
print(f"    SPY RSI14 (latest): {float(bench_rsi14.iloc[-1]):.2f}")
...
# Step 2b: Filter
print(f"\n[2b/5] Filtering  vol > {MIN_VOLUME:,}  AND  price > US${MIN_PRICE} ...")
...
for idx, (yf_t, df) in enumerate(filtered.items()):
    ...
    try:
        ...
        results.append({
            "symbol": info["symbol"], "name": info["name"], "sector": info.get("sector", ""),
...
print("\n" + "=" * 60)
print(f"  US CONFIRMED SIGNALS ({len(matched)} stocks)")
print("=" * 60)
if matched:
...
print(f"\n[5/5] FA scan on {len(matched)} confirmed US stocks...")
...
print(f"  RSI report : {OUTPUT_RSI}  (vs SPY)")
print(f"  Benchmark  : {BENCHMARK_DATA_DIR}/SPY.csv")
print(f"  FA reports : {FA_REPORTS_DIR}/  ({len(fa_reports)} HTML)")
print(f"  FA PDF     : {COMBINED_FA_PDF}")
print("=" * 60)
print("\nImport into TradingView: Watchlist → ⋮ → Import → us_watchlist.txt")
print("RSI ratio > 1.0 → stock stronger than SPY")
