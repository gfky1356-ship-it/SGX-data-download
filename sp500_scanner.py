# ============================================================
# S&P 500 STOCK SCANNER — Colab Ready  (fully self-contained)
#
# ── Quick Start (paste into Google Colab) ───────────────────
# Public repo (no token needed):
#   !wget -qO sp500_scanner.py https://raw.githubusercontent.com/gfky1356-ship-it/sgx-data-download/main/sp500_scanner.py && pip install yfinance requests -q
...
#
# Scanners (all 3 embedded):
#   1. SMA Gap Enlarge Buy
#   2. Breakout Buy
#   3. OBV (sell_ban confirmation)
#
# Signal confirmed if:
#   (sma_gap_buy AND obv_sell_ban) OR (breakout_buy AND obv_sell_ban)
#
# FA data: Yahoo Finance primary, price from downloaded batch data.
#
# Outputs:
...
import csv, io, os, time, warnings, webbrowser
import numpy as np
import pandas as pd
import requests
...
def run_fa_scan(matched):
    ...
    for r in matched:
        ...
        try:
            fa_data  = _fetch_fa_data(r["symbol"], r["name"])
            # Price from batch-downloaded data (last_close), not live fetch
            ...
            print(f"✅  {verdict_label}  [Price:{r['last_close']}]")
        except Exception as e:
            failed.append(r["symbol"])
            print(f"❌  {e}")
        time.sleep(0.5)
    print(f"\n    FA reports : {len(saved)} saved to {FA_REPORTS_DIR}/")
    if failed:
...
print("=" * 60)
print("\n[1/5] Fetching S&P 500 tickers from Wikipedia...")
...
try:
    ...
    for _, row in sp500_df.iterrows():
        yf_sym = str(row["Symbol"]).replace(".", "-")   # BRK.B → BRK-B
        ticker_info[yf_sym] = {
            "symbol": yf_sym,
            "name":   str(row.get("Security", yf_sym)),
...
# ════════════════════════════════════════════════════════════
# STEP 2 — Download 1-year price history + benchmark
# ════════════════════════════════════════════════════════════
...
if spy_raw.empty or len(spy_raw) < RSI_PERIOD + 5:
    print(f"    RSI index   : SPY download failed — falling back to ^GSPC")
    rsi_index_close  = bench_close
    rsi_index_label  = "^GSPC (fallback)"
else:
    rsi_index_close = spy_raw["Close"].squeeze()
    rsi_index_label = "SPY"
    last_bar   = rsi_index_close.index[-1]
    last_close = float(rsi_index_close.iloc[-1])
    print(f"    RSI index   : {rsi_index_label}  |  {len(rsi_index_close)} bars  "
          f"|  last bar: {last_bar.date()}  close: {last_close:.2f}")
...
# ════════════════════════════════════════════════════════════
# STEP 2b — Filter by volume and price
# ════════════════════════════════════════════════════════════
...
filtered = {t: df for t, df in stock_data.items()
            ...
            and float(df["close"].iloc[-1]) > MIN_PRICE}
...
for idx, (sym, df) in enumerate(filtered.items()):
...
print(f"    {rsi_index_label} RSI-{RSI_PERIOD}: {index_rsi_val:.2f}")
...
if rsi_excluded:
    print(f"    Excluded ({rsi_index_label} RSI {index_rsi_val:.1f} + {RSI_THRESHOLD} threshold):")
...
with open(OUTPUT_RESULTS, "w", newline="", encoding="utf-8") as f:
    ...
    w.writeheader()
    w.writerows(sorted(results, key=lambda r: r["signal_confirmed"], reverse=True))
